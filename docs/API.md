...
Access-токен: JWT, HS256, время жизни `JWT_TTL` (по умолчанию 1 час). Срок хранится в клейме
`exp` внутри токена, отдельного поля в ответе нет.

Отдельного refresh-токена нет: сессия продлевается скользящим окном через
`POST /api/v1/auth/refresh`, который обменивает ещё живой токен на новый. Фронтенд делает это
автоматически, поймав первый 401. Истёкший токен продлить нельзя — тогда требуется повторный
вход (см. «[Ограничения MVP](../README.md#ограничения-mvp)» в README).

### POST /api/v1/auth/register

Регистрация. Публичный эндпоинт. Побочный эффект: публикуется событие `user.registered`, по
которому pet-модуль заводит пользователю питомца первого уровня.
...
| `stage` | Стадия роста: `baby`, `teen`, `adult`, `legend` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| `stage` | string | `baby`, `teen`, `adult`, `legend` |
...
| `hatched_at` | timestamp | Момент создания питомца |
...
- **Токен живёт `JWT_TTL` (по умолчанию 1 час).** После истечения соединение нужно переустановить
  с новым токеном — например, полученным через `POST /api/v1/auth/refresh`.
