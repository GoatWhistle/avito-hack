...
Docker и Docker Compose, nginx как раздатчик статики и обратный прокси, golangci-lint
и ESLint + Prettier для статического анализа, pre-commit для локальных проверок.
...
Мини-игры — способ вернуть пользователя в приложение и связать развлечение с площадкой.
Все четыре игры построены на **реальных объявлениях**: игрок сравнивает цены, угадывает
слова из названий, а после раунда попадает прямо к товарам. Ежедневная игра засчитывается
в **общий недельный стрик** — за серию из 7 дней даётся бесплатное продвижение объявления.

Игры подключаются через единый реестр (`features/games/registry.ts`) и общий движок раундов:
добавить новую — это одна запись в реестре плюс компонент, без правок в остальных играх.
Античит серверный: счёт и прогресс проверяются на бэкенде, подсмотреть ответ или накрутить
результат нельзя.

### Больше или меньше

Показываются два реальных объявления: у одного цена известна, у второго скрыта до ответа.
Нужно угадать, дороже второе или дешевле. Пары подбираются близкими по цене — без халявных
«квартира против шапки». Скрытая цена и её объявление не отдаются клиенту до ответа, поэтому
подсмотреть в каталоге невозможно.

<p align="center">
  <img src="./assets/game-moreless.png" width="100%" alt="Мини-игра «Больше или меньше»">
</p>

### Буковки

Игра-угадайка слов на объявлениях Авито. За 6 попыток нужно угадать слово, загаданное из
названия реального объявления, — длина слова заранее неизвестна. Зелёный: буква на своём
месте, жёлтый: есть в слове, красный: нет. Слова берутся динамически из названий, а не из
хардкода; загаданное слово и проверка живут на сервере. После раунда показываются объявления
с этим словом в названии.

<p align="center">
  <img src="./assets/game-bukovki.png" width="100%" alt="Мини-игра «Буковки»">
</p>

### Прыжки Ноти

Аркада с талисманом сервиса: помогите еноту Ноти запрыгнуть по платформам как можно выше.
Пружины подбрасывают выше, хрупкие платформы ломаются. Физика, коллизии и генерация платформ
считаются на клиенте от серверного сида, а счёт валидируется на сервере по времени раунда —
результат нельзя накрутить ожиданием. По набранному счёту в конце открываются объявления.

<p align="center">
  <img src="./assets/game-notijump.png" width="100%" alt="Мини-игра «Прыжки Ноти»">
</p>

### Лотерея недели

Раз в неделю игроку выдаётся поле 3×3, на котором спрятана одна тройка одинаковых элементов.
Открывая ячейки, игрок собирает тройку и получает промокод на скидку в выпавшей категории.
Раскладка поля генерируется на сервере, а символы в закрытых ячейках не утекают клиенту —
исход честный и предсказать его заранее нельзя.

<p align="center">
  <img src="./assets/game-lottery.png" width="100%" alt="Мини-игра «Лотерея недели»">
</p>
...
- Тесты бэкенда — `go test -race` с профилем покрытия; тесты фронтенда — Vitest с MSW вместо
  ...
  функций). Порог 80% проверяется в `make lint`.
...
| [Михаил Хорохорин](https://github.com/mikhailkhorokhorin) | Фуллстек и инфраструктура | Модули `user`, `item`, `favorite`, загрузка фотографий и защищённая раздача, миграции и демо-данные, Docker Compose для разработки и продакшена, nginx и HTTPS, линтеры и проверки качества, документация |
