...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl bg-card py-(--card-spacing) text-sm text-card-foreground border border-border/60 shadow-sm transition-shadow hover:shadow-md [--card-spacing:--spacing(5)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3.5)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl',
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex items-center rounded-b-2xl border-t border-border/50 bg-transparent p-(--card-spacing)',
        className,
      )}
      {...props}
    />
