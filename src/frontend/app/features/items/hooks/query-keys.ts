...
export type ItemListFilters = Pick<ListItemsParams, 'status' | 'search'>

export const itemKeys = {
  all: ['items'] as const,
  lists: () => [...itemKeys.all, 'list'] as const,
  list: (filters: ItemListFilters) =>
    [...itemKeys.lists(), filters.status ?? '', filters.search ?? ''] as const,
  mine: (filters: ItemListFilters) =>
    [...itemKeys.all, 'mine', filters.status ?? ''] as const,
