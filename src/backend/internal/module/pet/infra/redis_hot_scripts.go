...
var nudgeHotState = redis.NewScript(`
local version = tonumber(redis.call('HGET', KEYS[1], 'version') or '-1')
local initial_version = tonumber(ARGV[4])
if version < initial_version then
  redis.call('HSET', KEYS[1],
...
if applied then
  ...
  local updated = math.min(tonumber(ARGV[10]), current + tonumber(ARGV[9]))
  redis.call('HINCRBY', KEYS[1], 'version', 1)
  redis.call('HSET', KEYS[1], field, updated, 'updated_at', ARGV[6])
